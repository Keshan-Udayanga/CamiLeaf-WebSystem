...
export default function ProductSummary() {
  ...
  const generatePDF = () => {
    ...
    try {
      ...
      doc.setFontSize(10); // Smaller subtitle
      doc.setFont('helvetica', 'normal');
      doc.text("Comprehensive Product Summary & Analysis", pageWidth / 2, 25, { align: 'center' }); 
      
      doc.setFontSize(9);
      doc.text(`Generated on: ${new Date().toLocaleDateString()}`, pageWidth / 2, 32, { align: 'center' });

      // Move summary section closer to header
      doc.setFillColor(248, 250, 252);
      doc.roundedRect(14, 45, pageWidth - 28, 30, 3, 3, 'F'); // Moved up from 65 to 45
      doc.setDrawColor(226, 232, 240);
      doc.roundedRect(14, 45, pageWidth - 28, 30, 3, 3, 'S');
      
      doc.setTextColor(30, 41, 59);
      doc.setFontSize(11); // Slightly smaller
      doc.setFont('helvetica', 'bold');
      doc.text("SUMMARY OVERVIEW", 20, 55); // Adjusted position
      
      doc.setFontSize(9); // Smaller text
      doc.setFont('helvetica', 'normal');
      doc.text(`Total Products: ${report.totalProducts || 0}`, 20, 62);
      doc.text(`Total Stock: ${report.totalStock || 0}`, 20, 68);
      // doc.text(`Categories: ${Object.keys(categoryCount).length}`, 100, 62);
      // doc.text(`On Discount: ${report.records?.filter(p => p.discount > 0).length || 0}`, 100, 68);

      // Move table start position higher
      const tableRows = report.records.map((product, index) => [
        index + 1,
        product.productName || "-",
        product.category || "-",
        product.price != null ? `Rs: ${product.price.toFixed(2)}` : "-",
        product.stock != null ? product.stock : "-",
        product.discount != null ? `${product.discount}%` : "-",
      ]);

      // Start table at higher position (90 → 80)
      autoTable(doc, {
        startY: 80, // Reduced from 90
        head: [["#", "Product Name", "Category", "Price", "Stock", "Discount"]],
        body: tableRows,
        styles: { 
          fontSize: 9,
          cellPadding: 4,
